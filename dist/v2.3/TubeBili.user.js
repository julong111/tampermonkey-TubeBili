...
`;
  ...
  var i18n = {
    zh: {
      ...
      Bilibili_DisplayMode_Enabled: "自动显示模式",
      Bilibili_DisplayMode_WebFullscreen: "网页全屏",
      Bilibili_DisplayMode_Wide: "宽屏模式",
    ...
    en: {
      ...
      Bilibili_DisplayMode_Enabled: "Auto Display Mode",
      Bilibili_DisplayMode_WebFullscreen: "Web Fullscreen",
      Bilibili_DisplayMode_Wide: "Wide Mode",
  ...
  function createDisplayModeItem(item) {
  ...
  function saveSettings() {
    ...
    for (const [key, item] of Object.entries(settingPanelItems)) {
      if (item.type === "display-mode" && item.radioName) {
        const selectedRadio = document.querySelector(`input[name="${item.radioName}"]:checked`);
        if (selectedRadio) {
    ...
    if (!settingPanelInitialized) {
      ...
      for (const [key, item] of Object.entries(settingPanelItems)) {
        if (item.type === "display-mode" && item.radioName) {
          ...
          const selectedRadio = document.querySelector(`input[name="${item.radioName}"][value="${savedValue}"]`);
          if (selectedRadio) {
  ...
  var YOUTUBE_AD_PLAY_DURATION = 1e4;
  ...
  var youtubeHandlers = {
    initListeners() {
      const settingPanelItems = getSettingPanelItems();
      const skipAdEnableKey = settingPanelItems.Youtube_Action_SkipAd?.enableKey;
          ...
          if (skipAdEnableKey && gm.getValue(skipAdEnableKey, false) && youtubeState.skipAdCheckInterval === null) {
            youtubeState.skipAdCheckInterval = setInterval(() => {
              ...
              if (skipAdOverlay) {
                ...
                if (adVideo && Number.isFinite(adVideo.duration) && adVideo.duration > 0 && youtubeState.adStartTime !== null && Date.now() - youtubeState.adStartTime >= YOUTUBE_AD_PLAY_DURATION) {
  ...
  function onFullscreenControlClick(e) {
    const mode = getDisplayMode();
    if (mode === "fullscreen" && e.target?.closest?.(bilibiliSelectors.fullScreenBtn) || mode === "web-fullscreen" && e.target?.closest?.(bilibiliSelectors.webFullBtn) || mode === "wide" && e.target?.closest?.(bilibiliSelectors.wideBtn)) {
      markUserExitMode();
    }
  }
  ...
  function setupWideModeTracking() {
  ...
  var bilibiliAdapter = definePlatformAdapter({
    ...
    onPage: () => {
      handleBilibiliPage();
      const settingPanelItems = getSettingPanelItems();
      const enabled = gm.getValue(settingPanelItems.Bilibili_DisplayMode_Enabled?.enableKey, false);
